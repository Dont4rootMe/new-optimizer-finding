...
- Keep task mechanics in `evolver/` and model-serving identity/routing in
  `backbone/`. A task must be able to switch every LLM stage with one
  `backbone=<profile>` override; do not copy route IDs, provider token limits,
  or named pipelines back into task configs.
...
- Optimization-survey builtin optimizer defaults must stay compatible with
  `experiments/optimization_survey/_runtime/runner.py`.
