...
class RunResult:
    ...
    objective_last: float
    objective_best: float
    objective_best_step: int
    first_step_at_or_below_baseline: int | None
    seed: int
    device: str
    precision: str
...
_REQUIRED_FIELDS: tuple[tuple[str, type | tuple[type, ...]], ...] = (
    ...
    ("objective_name", str),
    ("objective_direction", str),
    ("objective_last", (int, float)),
    ("objective_best", (int, float)),
    ("objective_best_step", int),
    ("seed", int),
    ("device", str),
    ("precision", str),
...
def validate_run_result_dict(data: dict[str, Any]) -> None:
    ...
    if status not in {"ok", "skipped", "failed", "interrupted"}:
        raise ValueError("Run result field 'status' must be one of: ok, skipped, failed, interrupted.")

    if "smoke" in data and not isinstance(data["smoke"], bool):
        raise ValueError("Run result field 'smoke' must be a bool when provided.")

    if "first_step_at_or_below_baseline" in data and data["first_step_at_or_below_baseline"] is not None and not isinstance(
        data["first_step_at_or_below_baseline"], int
    ):
        raise ValueError("Run result field 'first_step_at_or_below_baseline' must be int or null.")
