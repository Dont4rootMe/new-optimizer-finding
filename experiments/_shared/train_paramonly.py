"""Training loop for parameter-only experiments (no data batches).

Used by experiments where the loss is computed directly from model parameters
(e.g. Rosenbrock, quadratic bowl, matrix factorization).
The model's ``forward()`` must return a scalar loss.
"""
...
@torch.no_grad()
def evaluate_paramonly(
    cfg: DictConfig, model: nn.Module, datamodule: dict[str, Any],
) -> dict[str, float]:
    """Evaluate by running model.forward() which returns a scalar loss."""
    model.eval()
...
def train(
    ...
    """Run a parameter-only training loop.

    The model's forward() returns a scalar loss. There are no data batches.
    """
    ...
    try:
        while step < max_steps:
            ...
            if step % eval_every == 0 or step == max_steps:
                metrics = objective_tracker.attach_train_loss(evaluate_paramonly(cfg, model, datamodule))
    ...
    if not final_metrics:
        if status == "ok":
            final_metrics = objective_tracker.attach_train_loss(evaluate_paramonly(cfg, model, datamodule))
        else:
