"""Generic supervised training loop for simple experiments.

Supports both classification (CrossEntropyLoss + accuracy) and regression
(MSELoss + loss-only) via the ``eval_mode`` parameter.
"""
...
def evaluate_classification(
    cfg: DictConfig, model: nn.Module, datamodule: dict[str, Any],
) -> dict[str, float]:
    """Evaluate a classification model (CrossEntropyLoss + accuracy)."""
...
def evaluate_regression(
    ...
    """Evaluate a regression model (MSELoss only)."""
    device = _device_from_cfg(cfg)
    model.eval()
...
EvalFn = Callable[[DictConfig, nn.Module, dict[str, Any]], dict[str, float]]
...
def train(
    ...
    optimizer_factory,
    *,
    criterion_cls: type[nn.Module] = nn.CrossEntropyLoss,
    eval_fn: EvalFn = evaluate_classification,
) -> dict[str, Any]:
    """Run a supervised training loop and return structured summary.

    Parameters
    ----------
    criterion_cls:
        Loss class to instantiate (e.g. ``nn.CrossEntropyLoss``, ``nn.MSELoss``).
    eval_fn:
        Callable ``(cfg, model, datamodule) -> metrics_dict``.
    """
    ...
    # Direction-aware best tracking
    direction = str(cfg.primary_metric.get("direction", "max"))
    is_better = (lambda a, b: a < b) if direction == "min" else (lambda a, b: a > b)
    best_primary: float = float("inf") if direction == "min" else -float("inf")
    ...
    try:
        while step < max_steps:
            ...
            if step % eval_every == 0 or step == max_steps:
                metrics = objective_tracker.attach_train_loss(eval_fn(cfg, model, datamodule))
                final_metrics = metrics
                primary_value = compute_primary(metrics, cfg.primary_metric)
                if is_better(primary_value, best_primary):
    ...
    if not final_metrics:
        if status == "ok":
            final_metrics = objective_tracker.attach_train_loss(eval_fn(cfg, model, datamodule))
        else:
