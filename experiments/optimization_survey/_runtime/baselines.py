"""Baseline profile helpers for the optimization-survey family."""
...
from experiments.optimization_survey._runtime.objective_tracking import safe_objective_float


def baseline_path(stats_root: str | Path, family_name: str, experiment_name: str) -> Path:
    """Return the canonical baseline path for one experiment."""

    return Path(stats_root).expanduser().resolve() / family_name / experiment_name / "baseline.json"
...
def load_baseline_profile(stats_root: str | Path, family_name: str, experiment_name: str) -> dict[str, Any]:
    """Load and validate the canonical baseline profile for one experiment."""

    path = baseline_path(stats_root, family_name, experiment_name)
    if not path.exists():
...
def inject_baseline_threshold(
    ...
    *,
    ...
    baseline_file = baseline_path(stats_root, family_name, experiment_name)
    ...
    try:
        payload = load_baseline_profile(stats_root, family_name, experiment_name)
