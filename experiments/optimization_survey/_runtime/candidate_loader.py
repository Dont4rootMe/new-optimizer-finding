"""Dynamic import helpers for optimization-survey organism implementations."""
...
def _validate_signature(
    ...
    *,
    ...
    try:
        ...
        raise TypeError(
            f"Could not inspect {label} in candidate module '{module_path}': {exc}"
        ) from exc
    ...
    if actual != expected_params:
        ...
        raise TypeError(
            f"Candidate module '{module_path}' must define {label}"
...
def import_module_from_path(path: str) -> ModuleType:
    ...
    module_name = f"optimization_survey_external_{hashlib.sha1(str(module_path).encode('utf-8')).hexdigest()[:12]}"
...
def _validate_controller(controller: Any, module_path: Path) -> None:
    ...
    if missing:
        ...
        raise AttributeError(
            f"Candidate module '{module_path}' returned invalid controller. "
            f"Missing callable methods: {missing_str}."
        )


def load_candidate_builder(path: str) -> tuple[OptimizerBuilder, Path, str]:
    """Load strict optimization-survey `build_optimizer(model, max_steps)` contract."""
    ...
    if build_optimizer is None or not callable(build_optimizer):
        raise AttributeError(
            f"Candidate module '{module_path}' must define callable build_optimizer(model, max_steps)."
        )
    _validate_signature(
        build_optimizer,
    ...
    candidate_name_obj = getattr(module, "OPTIMIZER_NAME", module_path.stem)
    candidate_name = str(candidate_name_obj)
    ...
    return builder, module_path, candidate_name
