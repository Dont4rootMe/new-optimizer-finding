...
from valopt.optimizer_api import NamedParameters, OptimizerBuilder, OptimizerControllerProtocol
from valopt.registry import create_experiment
from valopt.schemas import OptionalDependencyError, RunResult, validate_run_result_dict
from valopt.utils.import_utils import load_optimizer_builder
from valopt.utils.io import ensure_dir, save_json, save_yaml_text
...
class ExperimentRunner:
    ...
    def _persist_result(
        ...
        if mode == "stats":
            stats_dir = ensure_dir(Path(self.cfg.paths.stats_root) / exp_name)
            result_path = stats_dir / f"baseline__{run_id}.json"
            save_json(result_path, result_payload)
            return result_path
