"""Async orchestrator for generation and GPU evaluation pipeline."""
...
from src.evolve.types import CandidateSummary, EvalTask, EvalTaskResult, GenerationSummary

LOGGER = logging.getLogger(__name__)


class EvolverOrchestrator:
    """Coordinates proposal generation and distributed evaluations."""
    ...
    def __init__(self, cfg: DictConfig) -> None:
        ...
        self.expected_experiments = [str(name) for name in self.evolver_cfg.eval_experiments]
    ...
    def _validate_eval_experiments(self) -> None:
        ...
        if missing:
            raise ValueError(f"Unknown evolver.eval_experiments entries: {missing}")
        ...
        if sample_size > len(self.expected_experiments):
            raise ValueError(
                "evolver.allocation.sample_size must be <= len(evolver.eval_experiments)"
            )
    ...
    def _default_allocation_snapshot(self) -> dict[str, Any]:
        uniform = 1.0 / float(len(self.expected_experiments))
        return {
            "method": "uniform",
            "enabled": False,
            "history_window": 0,
            "sample_size": len(self.expected_experiments),
            "pi": {exp_name: uniform for exp_name in self.expected_experiments},
            "stats": {
                exp_name: {"mean": 0.0, "std": 0.0, "n": 0, "cost": uniform}
                for exp_name in self.expected_experiments
            },
    ...
    def _build_task(self, candidate_id: str, exp_name: str) -> EvalTask:
        ...
        return EvalTask(
            ...
            mode=str(self.evolver_cfg.eval_mode),
            config_path="conf",
            entrypoint_module=str(self.evolver_cfg.eval_entrypoint_module),
            timeout_sec=int(self.evolver_cfg.timeout_sec_per_eval),
            max_retries=int(self.evolver_cfg.max_retries_per_eval),
    ...
    def _build_candidate_allocation(self, candidate_id: str) -> dict[str, Any]:
        ...
        selected = allocation_snapshot.get("selected_experiments", [])
        if not selected:
            allocation_snapshot = self._default_allocation_snapshot()
    ...
    async def _proposal_worker(self, worker_idx: int, proposal_queue: asyncio.Queue[Any], eval_queue: asyncio.Queue[Any]) -> None:
        while True:
            ...
            try:
                context = load_best_context(self.population_root, limit=3)
                ...
                fallback_allocation = self._default_allocation_snapshot()
    ...
    def _finalize_candidate_if_ready(self, candidate_id: str) -> None:
        ...
        aggregate_score, status, per_experiment = mean_score(
            ...
            allocation_pi=allocation_snapshot.get("pi", {}),
    ...
    async def run(self) -> dict[str, Any]:
        """Execute one configured generation run."""
        ...
        generated_candidates = len(self.candidate_states)
        completed_candidates = len(self.finished_summaries)

        ok_candidates = sum(1 for summary in self.finished_summaries if summary.get("status") == "ok")
        partial_candidates = sum(1 for summary in self.finished_summaries if summary.get("status") == "partial")
        failed_candidates = sum(1 for summary in self.finished_summaries if summary.get("status") == "failed")
        ...
        generation_summary = GenerationSummary(
            ...
            candidate_summaries=self.finished_summaries,
        )

        summary_payload = generation_summary.to_dict()
        summary_payload["resumed_candidates"] = resumed_candidates

        write_json(self.gen_dir / "generation_summary.json", summary_payload)
        self._append_event("generation_finished", summary_payload)
